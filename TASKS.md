...
- [x] 确定重复 gene symbol 使用 mean 聚合。
- [x] 过滤 368 个在所有样本中完全缺失的基因行。
- [x] 生成 `data/processed/expression_cleaned.csv` 和表达清洗报告。
- [x] 将 `merge_omics.py` 的 expression 输入切换为清洗后的表达矩阵。
- [x] 使用清洗后的 patient 和 expression 重新安全合并，结果为 `514 × 20201`。

## 下一最小任务

- [ ] 核验清洗后合并表的 56 个元数据列与 20145 个表达特征列边界。
- [ ] 明确后续实验必须通过训练集内部 Pipeline 处理可能出现的剩余缺失值。

## 后续任务

- [ ] 决定旧 `data_analysis.py` 的后续职责，避免与新 patient 清洗入口产生歧义。
- [ ] 在后续建模阶段定义训练集内部的缺失值 Pipeline；当前不做全局填补。
